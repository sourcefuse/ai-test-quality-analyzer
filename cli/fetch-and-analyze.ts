...
function getJiraConfig(): JiraConfigDto {
  ...
  return {
    ...
    maxResult: process.env.JIRA_MAX_RESULT
      ? parseInt(process.env.JIRA_MAX_RESULT)
      : undefined,
    fetchFields,
  };
}
...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (piiStatus.presidioConfigured) {
      console.log(
        `   Presidio configured: ${piiStatus.presidioStatus?.isAvailable ? "✅ Available" : "❌ Unavailable"}`,
      );
      ...
      console.log(
        "   ℹ️  Presidio not configured, using regex-based detection",
      );
    }
    ...
    if (ticketId && jiraConfig.projectKey) {
      ...
      if (process.env.SAVE_TO_FILE === "true") {
        ...
        const baseFolderSuffix =
          process.env.BASE_FOLDER_SUFFIX || "Quality-Check-Via-AI";
        const ticketFolderSuffix = process.env.TICKET_FOLDER_SUFFIX || "Via-AI";
        const timestampFolderSuffix =
          process.env.TIMESTAMP_FOLDER_SUFFIX || "Via-AI";
        ...
        if (currentAnalysisPath) {
          ...
          if (!fs.existsSync(tmpDir)) {
            ...
            console.log(
              `✅ Created analysis folder from CURRENT_ANALYSIS_PATH: ${tmpDir}`,
            );
          } else {
            console.log(
              `ℹ️  Using existing analysis folder from CURRENT_ANALYSIS_PATH: ${currentAnalysisPath}`,
            );
          }
        } else {
    ...
    if (confluenceConfig.spaceKey) {
      ...
      if (process.env.SAVE_TO_FILE === "true") {
        ...
        if (currentAnalysisPath) {
          ...
          if (fs.existsSync(ticketDir)) {
            ...
            const timestampPattern = new RegExp(
              `^\\d{4}-\\d{2}-\\d{2}-\\d{2}-\\d{2}-\\d{2}-${timestampFolderSuffix.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}$`,
            );
            ...
            if (timestampFolders.length > 0) {
              ...
              console.log(
                `ℹ️  Using existing analysis folder: ${timestampFolders[0]}`,
              );
            }
          }
        ...
        let ticketInfo = "";
        if (jiraContent) {
          const ticketIdMatch = jiraContent.match(
            /\*\*Ticket ID:\*\*\s*([A-Z]+-\d+)/,
          );
        ...
        const confluenceFileName =
          process.env.CONFLUENCE_FILE_NAME || "Confluence.md";
...
`;
        ...
        const skipConfluenceFetch =
          process.env.SKIP_CONFLUENCE_FETCH === "true";
        ...
        if (skipConfluenceFetch) {
          ...
          const timestampPagePattern = new RegExp(
            `^\\d{4}[-\\d]*-${timestampFolderSuffix.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}$`,
          );
          ...
          for await (const batch of confluenceService.listPagesInBatches(
            searchRequest,
          )) {
            batchCount++;
            if (!silentMode) {
            ...
            for (const page of batch.pages) {
              ...
              const matchesSuffix = excludeTitlePatterns.some(
                (pattern) =>
              ...
              if (isAIReport) {
                if (!silentMode) {
                  console.log(
                    `   🤖 Skipping AI-generated report: ${page.title}`,
                  );
                }
              ...
              if (!page.body?.storage?.value) {
                if (!silentMode) {
                  console.log(
                    `   ⊘ Skipping page without content (folder/container): ${page.title}`,
                  );
                }
                pagesWithoutContent++;
                totalPagesSkipped++;
              ...
              if (!cleanContent || cleanContent.trim().length === 0) {
                if (!silentMode) {
                  console.log(
                    `   ⊘ Skipping page with empty content: ${page.title}`,
                  );
                }
                pagesWithoutContent++;
                totalPagesSkipped++;
              ...
              if (piiResult.hasPII) {
                ...
                let piiCount = 0;
                if (
                  piiResult.method === "presidio" &&
                  piiResult.presidioEntities
                ) {
                  ...
                  for (const [type, count] of Object.entries(summary)) {
                    piiSummaryByType[type] =
                      (piiSummaryByType[type] || 0) + count;
                  }
                } else if (
                  piiResult.method === "regex" &&
                  piiResult.regexMatches
                ) {
                ...
                if (!silentMode) {
                  console.log(
                    `   🔒 ${page.id}: ${page.title} (${piiCount} PII item(s) redacted via ${piiResult.method})`,
                  );
                }
              } else {
                if (!silentMode) {
              ...
              collectedPages.push({
                ...
                url: page._links?.webui
                  ? `${confluenceConfig.host}${page._links.webui}`
                  : "",
              });
            }
            ...
            if (batchContent.trim().length > 0) {
              if (!silentMode) {
                console.log(
                  `   💾 Saving batch ${batch.batchNumber} content to file...`,
                );
              }
              const batchSeparator = `\n<!-- BATCH ${batch.batchNumber} - ${new Date().toISOString()} -->\n\n`;
              fs.appendFileSync(
                confluenceFilePath,
                batchSeparator + batchContent + "\n\n",
              );
              if (!silentMode) {
                console.log(
                  `   ✅ Batch ${batch.batchNumber} saved: ${pagesWithContent} page(s) with content${pagesWithoutContent > 0 ? `, ${pagesWithoutContent} skipped (no content)` : ""}`,
                );
              }
            } else {
              if (!silentMode) {
                console.log(
                  `   ⊘ Batch ${batch.batchNumber}: All ${pagesWithoutContent} page(s) skipped (no content)`,
                );
              }
            }
          }
          ...
          console.log(
            `Total pages skipped (no content): ${totalPagesSkipped - totalAIReportsSkipped}`,
          );
          console.log(
            `Total AI-generated reports excluded: ${totalAIReportsSkipped}`,
          );
...
${
  totalPIIDetected > 0
    ? `### PII Types Found:\n${Object.entries(piiSummaryByType)
...
`;
          ...
          if (totalPIIDetected > 0) {
            ...
            console.log(
              `   Pages with PII: ${totalPagesWithPII}/${totalPagesProcessed}`,
            );
            console.log(`\n📊 PII Types:`);
...
`;
        ...
        const usePostgresVectorDB =
          process.env.USE_POSTGRES_VECTOR_DB === "true";
        if (usePostgresVectorDB && collectedPages.length > 0) {
          ...
          try {
            ...
            const {
              ...
              ConfluenceIndexerService,
            } = await import("../src/services");
            const { getRequiredEnv, getOptionalEnvAsNumber } =
              await import("../src/utils/env-validator.util");
            ...
            const embeddingService = new EmbeddingService({
              ...
              provider:
                (process.env.EMBEDDING_PROVIDER as "openai" | "openrouter") ||
                "openai",
            ...
            const checkPgBeforeFetch =
              process.env.CHECK_PG_BEFORE_CONFLUENCE_FETCH !== "false"; // default true
            ...
            if (checkPgBeforeFetch && projectKey) {
              ...
              const existingCount =
                await vectorService.getDocumentCountByProjectKey(projectKey);
              console.log(
                `   Found ${existingCount} non-expired document(s) for project ${projectKey}`,
              );

              if (existingCount > 0) {
                shouldFetchConfluence = false;
                console.log(
                  `   ✅ Using existing data from PostgreSQL - skipping Confluence fetch`,
                );
              } else {
                console.log(
                  `   ℹ️  No existing data found - will fetch from Confluence`,
                );
              }
            }
            ...
            if (shouldFetchConfluence) {
              ...
              if (ticketId) {
                try {
                  ...
                  console.log(
                    `✅ Jira ticket fetched: ${jiraIssueForFilter.fields.summary}`,
                  );
                } catch (error) {
                  console.log(
                    `⚠️  Could not fetch Jira ticket for filtering: ${error}`,
                  );
              ...
              console.log(
                `   Max pages to fetch: ${maxPages > 0 ? maxPages : "All"}`,
              );
            ...
            console.error(
              "\n❌ Error pushing to PostgreSQL Vector DB:",
              error.message,
            );
            if (error.stack) {
            ...
            console.error(
              "   You can manually index to vector DB later using: npm run process:jira",
            );
          }
          ...
          console.log(
            "   This may be because cache was loaded or no pages had content",
          );
        } else {
          console.log(
          ...
          console.log(
            "   Set USE_POSTGRES_VECTOR_DB=true in .env to enable automatic indexing",
          );
        }
      } else {
      ...
      console.log(
        "   Set this variable in .env to test Confluence integration",
      );
    }
